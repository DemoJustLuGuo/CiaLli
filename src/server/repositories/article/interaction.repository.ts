...
export async function loadArticleInteractionSnapshotFromRepository(input: {
    articleId: string;
...
export async function invalidateArticleInteractionAggregateFromRepository(
    articleId: string,
): Promise<void> {
    const normalizedArticleId = normalizeIdentity(articleId);
    if (!normalizedArticleId) {
...
export async function invalidateArticleInteractionViewerStateFromRepository(
    articleId: string,
    viewerId: string,
): Promise<void> {
    const normalizedArticleId = normalizeIdentity(articleId);
